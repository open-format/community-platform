...
import { createServerWallet, getCurrentUser, getUserHandle } from "./privy";
...
const agentClient = axios.create({
  baseURL: config.OPENFORMAT_AGENT_URL,
  headers: {
    Authorization: `Bearer ${config.OPENFORMAT_AGENT_KEY}`,
  },
});
...
export async function fetchAgentWallet(communityId: string) {
  console.log("fetching agent wallet for community", communityId);
  try {
    const response = await agentClient.get(`/communities/${communityId}`);
    return response.data.communityWalletAddress;
  } catch (e) {
    console.error(e);
    return null;
  }
