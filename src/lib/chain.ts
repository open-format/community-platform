...
import { Config, readContract, waitForTransactionReceipt, writeContract } from "@wagmi/core";
import { Address, encodeFunctionData, erc20Abi, maxUint256, parseEther, stringToHex } from "viem";
...
export async function rewardTokenMint(config: Config, rewardTokenMintParams: RewardTokenMintParams) {
    return await executeTransaction(config, {
...
export async function rewardTokenTransfer(config: Config, rewardTokenTransferParams: RewardTokenTransferParams) {
    ...
    return await executeTransaction(config, {
        ...
        args: [
            ...
            rewardTokenTransferParams.metadata?.length > 0 ? stringToHex(rewardTokenTransferParams.metadata) : "",
        ],
    });
}

async function createAllowance(config: Config, rewardTokenTransferParams: RewardTokenTransferParams) {
...
async function encodeAllowanceCall(config: Config, rewardTokenTransferParams: RewardTokenTransferParams) {
    const allowance = await getAllowance(config, rewardTokenTransferParams);
    if (allowance < parseEther(rewardTokenTransferParams.amount.toString())) {
        return encodeFunctionData({
            abi:                    erc20Abi,
