...
export const community_roles = pgTable(
	...
	(table) => [
		uniqueIndex("role_community_unique_idx").on(table.communityId, table.name),
	],
);
...
export const communities = pgTable("communities", {
	...
	accentColor: varchar("accent_color", { length: 7 })
		.notNull()
		.default("#6366F1"),
	tokenLabel: varchar("token_label", { length: 255 })
		.notNull()
		.default("Points"),
	userLabel: varchar("user_label", { length: 255 }).notNull().default("User"),
	participantLabel: varchar("participant_label", { length: 255 })
		.notNull()
		.default("Participant"),
...
export const user_communities = pgTable(
	...
	(table) => [
		uniqueIndex("user_community_unique_idx").on(
			table.userId,
			table.communityId,
		),
	],
...
export const user_community_roles = pgTable(
	...
	(table) => [
		uniqueIndex("user_community_role_unique_idx").on(
			table.userId,
			table.communityId,
			table.roleId,
		),
...
export const userCommunityRolesRelations = relations(
	user_community_roles,
	({ one }) => ({
		user: one(users, {
