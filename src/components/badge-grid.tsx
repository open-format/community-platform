...
import { addressSplitter, getContrastSafeColor } from "@/lib/utils";
...
export default function BadgeGrid({
  badges,
  communityId,
}: { badges: Badge[] | undefined; communityId: string }) {
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3 lg:grid-cols-4">
        <Card className="border-dashed">
          <CardContent className="mt-5">
            <AspectRatio
              ratio={1 / 1.2}
              className="border-2 border-primary rounded-md flex flex-col items-center justify-center "
            >
              <p
                className="text-center text-muted-foreground px-4"
                style={{ color: getContrastSafeColor("#000000") }}
              >
                {t("createFirst")}
              </p>
            </AspectRatio>
...
function Item({
  ...
  metadataURI,
  communityId,
}: { badge: Badge; metadataURI: string; communityId: string }) {
    ...
    <Card>
      <CardContent className="mt-5">
        <AspectRatio ratio={1 / 1.2} className="bg-muted rounded-md">
          {image ? (
            <Image src={image} alt={metadata?.name} fill className="rounded-md object-cover" />
          ) : (
            <Skeleton />
          )}
        </AspectRatio>
        ...
        {metadata?.name ? (
          <div className="flex flex-col justify-between w-full space-y-2">
            ...
            <CardDescription className="flex">
              {t("address")} <span>{addressSplitter(badge.id || "")}</span>
              <CopyIcon className="mx-2 cursor-copy" onClick={copyBadgeAddress} />
              <Link
                href={`${chain?.BLOCK_EXPLORER_URL}/token/${badge.id}`}
                target="_blank"
